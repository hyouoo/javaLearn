...
public class Q38 {
    ...
    private static int[] solution(int[] array, int[][] commands) {
        ...
        int index = 0;
        for (int[] eachCom : commands) {
            list.clear();
            for (int i = eachCom[0] - 1; i < eachCom[1]; i++) {
                list.add(array[i]);
            }
            list.sort(Comparator.naturalOrder());
            result[index++] = list.get(eachCom[2] - 1);
        }
        return result;
    }
    ...
    private static int[] solution3(int[] array, int[][] commands) {
        int n = 0;
        int[] result = new int[commands.length];
