...
public class Q50 {
    ...
    private static int solution(int number, int limit, int power) {
        // 각 숫자별로 약수의 갯수를 구하고 공격력을 산출, if limit보다 크면 power로 반환
        int powerSum = 0;
        for (int i = 1; i <= number; i++) {
            int countDivisor = countDivisor(i);
            powerSum += countDivisor > limit ? power : countDivisor;
        }
        return powerSum;
    }

    static int countDivisor(int number) {
        int count = 0;
    ...
    private static int solution2(int number, int limit, int power) {
